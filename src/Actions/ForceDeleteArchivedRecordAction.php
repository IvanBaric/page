...
final class ForceDeleteArchivedRecordAction
{
    ...
    public function handle(string $type, string $uuid): ActionResult
    {
        ...
        DB::transaction(function () use ($type, $uuid): void {
            $this->resolveArchivedRecord($type, $uuid, lock: true)?->forceDelete();
        });

        return ActionResult::success(__('Zapis je trajno obrisan.'));
    }
}
